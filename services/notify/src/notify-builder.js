...
module.exports = {
	...
	setTemplateVariable(key, value, redact = true) {
		logger.debug({ key, value: redact ? 'redacted' : value }, 'Setting template personalisation variable.');
	...
	setTemplateVariablesFromObject(templateVariableObject) {
		...
		Object.entries(templateVariableObject).forEach(([key, value]) => this.setTemplateVariable(key, value));
	...
	async sendEmail() {
		...
		logger.debug({
			...
			templatePersonalisation: `Has ${Object.keys(this.templatePersonalisation || {}).length} value(s) set.`,
		...
		try {
			...
			if (!this.destinationEmail) {
				throw new Error('Notify: A destination email address must be set before an email can be sent.');
			}
