	...
	describe('NotifyBuilder', () => {
		...
		describe('getNotifyClient', () => {
			...
			test('can build and send an email', async () => {
				...
				expect(logger.info.mock.calls).toEqual([['Resetting the notify client'], [`Sending email via Notify`]]);
		...
		describe('sendEmail', () => {
			describe('guards', () => {
				...
				test('logs and throws if reference is not set', async () => {
					...
					try {
						await NotifyBuilder.setTemplateId('123').setDestinationEmailAddress('abc@example.com').sendEmail();
					} catch (e) {
						expect(logger.error).toHaveBeenCalledWith({ err: error }, 'Notify: error sending email');
						expect(e).toEqual(error);
					}
		...
		describe('addFileToTemplateVariables', () => {
			test('prepares the upload', () => {
				...
				NotifyBuilder.setNotifyClient(mockCreateNotifyClient).addFileToTemplateVariables('file key', fileValue);
			...
			[
				...
				test(description, async () => {
					...
					try {
						...
						expect(mockCreateNotifyClient.sendEmail).toHaveBeenCalledWith(templateId, destinationEmail, {
							personalisation: templatePersonalisation,
							reference,
							emailReplyToId
						});
