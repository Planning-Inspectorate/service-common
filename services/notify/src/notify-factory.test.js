  ...
  describe("getNotifyClientArguments", () => {
    describe("Using mock service ", () => {
      ...
      test("throws if `serviceId` is undefined", () => {
        expect(() =>
          getNotifyClientArguments(baseUrl, undefined, apiKey)
        ).toThrow("If baseUrl is provided, serviceId must be provided also.");
      });
    });
  ...
  describe("createNotifyClient", () => {
    test("can explicitly specify config", () => {
      expect(
        ...
        new NotifyClient(
          ...getNotifyClientArguments(baseUrl, serviceId, apiKey)
        )
      );
    });
